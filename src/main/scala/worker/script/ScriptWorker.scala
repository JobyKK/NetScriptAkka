...
object ScriptWorker {
  ...
  def props(clusterClient: ActorRef, workExecutorProps: Props, registerInterval: FiniteDuration = 10.seconds): Props =
    Props(classOf[ScriptWorker], clusterClient, workExecutorProps, registerInterval)

  case class WorkComplete(result: Any)
}

class ScriptWorker(clusterClient: ActorRef, workExecutorProps: Props, registerInterval: FiniteDuration)
  extends Actor with ActorLogging {
  import ScriptWorker._
  ...
  def working: Receive = {
    ...
    case _: Work =>
      log.info("Yikes. master.Master told me to do work, while I'm working.")
  }
